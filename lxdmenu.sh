...
LXDsetup(){
	...
	lastmessage="---LXD installed. RESTART REQUIRED for group permissions.---"
}
...
LXDmenu(){
	MenuTitle=" LXD - Options (Snap on Pop!_OS)"

LXD_menu(){
	echo -e "\n $MenuTitle \n-----------------------------------"
	echo "10. Install LXD (Snap)        20. List All (Detailed)"
	echo -e "${ORANGE}30. Create Ubuntu 22.04         31. Create Ubuntu 20.04 ${STD}"
	echo -e "${ORANGE}32. Create App Server           45. GitLab / 44. GoLang${STD}"
    echo "40. Nginx      41. Gogs       42. ROR        46. Ruby"
	...
	echo -e "${MAGENTA}97. Restart Container           99. Exit${STD}"
	echo "-------------------------------" 
    echo -e " Status: $lastmessage " 
    echo "-------------------------------"
	
	# RESTORED RUNNING CONTAINERS LOGIC
	RunningList=$(lxc list status=running --format csv -c n)
	if [ -z "$RunningList" ]; then
		echo -e "${RED}No running containers.${STD}"
	else
