...
echo -e "${GREEN}Installing SAERT (v20.0 - Universal Graphics Edition) to $TARGET_PATH...${STD}"
...
# Version: 20.0 (Universal Graphics Repair - NVIDIA/AMD/Intel)
# Description: Admin Dashboard with specialized fixes for all GPU vendors.
# ==========================================================
...
InstallCoreTools(){
    echo -e "${CYAN}--- Installing All Core & Rescue Tools (v20.0) ---${STD}"
...
# --- NEW: GRAPHICS RESCUE FUNCTIONS (v20.0) ---
...
Graphics_Menu(){
    while true; do
        ...
        echo " "
        echo -e " ${CYAN}1.${STD} ${GREEN}NVIDIA REPAIR:${STD} Auto-Install Recommended Drivers"
        echo -e " ${CYAN}2.${STD} ${GREEN}NVIDIA REPAIR:${STD} Purge Drivers (Force fallback to Nouveau)"
        echo -e " ${CYAN}3.${STD} ${MAGENTA}AMD REPAIR:${STD} Purge AMDGPU-PRO (Fix broken proprietary installs)"
        echo -e " ${CYAN}4.${STD} ${BLUE}UNIVERSAL REPAIR:${STD} Reinstall Open Source Stack (Mesa/Intel/AMD)"
        echo -e " ${CYAN}5.${STD} ${WHITE}CONFIG REPAIR:${STD} Delete Xorg Config (Reset Display Settings)"
        echo -e " ${CYAN}6.${STD} ${WHITE}GUI RESTART:${STD} Restart Display Manager (GDM/LightDM)"
        echo " "
        ...
        case $g_choice in
            1) 
                ...
                echo "This attempts to identify your card (NVIDIA/Wifi) and install the distro-tested driver."
            ...
            3)
                echo -e "${MAGENTA}AMD REPAIR: Attempting to remove AMDGPU-PRO...${STD}"
                echo "This is only needed if you manually installed the proprietary 'Pro' driver and it broke boot."

echo "--- Preparing to Uninstall AMD Drivers ---"

# 1. Backup your current OS Identity
if [ ! -f /etc/os-release.pop_backup ]; then
    echo "Backing up Pop!_OS identity..."
    sudo cp /etc/os-release /etc/os-release.pop_backup
fi

# 2. Switch Identity to Ubuntu
echo "Spoofing OS ID to 'ubuntu'..."
sudo sed -i 's/ID=pop/ID=ubuntu/g' /etc/os-release
sudo sed -i 's/ID_LIKE="ubuntu debian"/ID_LIKE=debian/g' /etc/os-release

# 3. Run the AMD Uninstaller
echo "Running AMD Uninstaller..."
if command -v amdgpu-install &> /dev/null; then
    sudo amdgpu-install --uninstall
elif command -v amdgpu-pro-uninstall &> /dev/null; then
    sudo amdgpu-pro-uninstall
else
    echo "Standard uninstall scripts not found. Attempting apt purge..."
    sudo apt purge "amdgpu*" "rocm*" -y
fi

# 4. Restore Pop!_OS Identity (CRITICAL)
echo "Restoring Pop!_OS identity..."
sudo cp /etc/os-release.pop_backup /etc/os-release
...
echo "Done. Please REBOOT your computer now."
                pause ;;
            4)
                echo -e "${BLUE}UNIVERSAL REPAIR: Reinstalling Xorg and Mesa...${STD}"
                echo "This fixes corruption in Intel, AMD, and basic display libraries."
                # Reinstalls the entire open source graphical stack
                sudo apt install --reinstall xserver-xorg-video-all xserver-xorg-core libgl1-mesa-dri libgl1-mesa-glx -y
                echo "Reinstall complete. Reboot recommended."
                pause ;;
            5)
...
# -----------------------------------------------------------
# 📦 GENERIC PODMAN MANAGER FUNCTIONS (Abbreviated for length, logic same as v18)
# -----------------------------------------------------------
...
Podman_Menu(){
    ...
    while true; do
        clear
        echo -e "${BLUE}╔════════════════════════════════════════════════════════════════════════════════════════════════════╗${STD}"
        echo -e "${BLUE}║${STD} ${WHITE}PODMAN CONTAINER MANAGER V20.0${STD}                                                         ${BLUE}║${STD}"
        echo -e "${BLUE}╠════════════════════════════════════════════════════════════════════════════════════════════════════╣${STD}"
...
ShowMenu(){
    ...
    echo -e "  ${WHITE}24.${STD} ${RED}GRAPHICS REPAIR (NVIDIA/AMD/Intel) [NEW]${STD}"
