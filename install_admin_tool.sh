...
Graphics_Menu(){
    while true; do
        ...
        case $g_choice in
            ...
            3) if command -v amdgpu-install &> /dev/null; then sudo amdgpu-install --uninstall; else sudo apt purge "amdgpu-pro*" -y; fi; pause ;;
            4) sudo apt install --reinstall xserver-xorg-video-all xserver-xorg-core libgl1-mesa-dri libgl1-mesa-glx -y; pause ;;
            5) if [ -f /etc/X11/xorg.conf ]; then sudo rm /etc/X11/xorg.conf; fi; pause ;;
            6) if systemctl is-active --quiet gdm; then sudo systemctl restart gdm; fi
...
CreateNewProject(){ read -rp "Project Name: " newproject; mkdir -p "$HOME/container_projects/$newproject"; cd "$HOME/container_projects/$newproject" 2>/dev/null; loadproject="$newproject"; lastmessage="Project created: $newproject"; }
LoadProject(){ ls -d "$HOME/container_projects"/*/ 2>/dev/null | xargs -n 1 basename; read -rp "Project Name: " loadproject; cd "$HOME/container_projects/$loadproject" 2>/dev/null; lastmessage="Loaded: $loadproject"; }
...
GetPodmanStats(){ local R=$(sudo podman ps -q 2>/dev/null | wc -l); local I=$(sudo podman images -q 2>/dev/null | wc -l); local P=$(sudo podman pod list -q 2>/dev/null | wc -l); echo -e "${GREEN}Running: ${R:-0}${STD} | ${CYAN}Images: ${I:-0}${STD} | ${MAGENTA}Pods: ${P:-0}${STD}"; }
...
ShowMenu(){
    ...
    printf " ${WHITE}%-2s${STD} %-35s ${WHITE}%-2s${STD} %-35s\n" "21." "Run Boot-Repair (GUI)" "24." "${RED}GRAPHICS REPAIR (GPU/Xorg)${STD}"
    ...
    lastmessage=""
    case $choice in
        10) InstallCoreTools ;; 11) SystemUpdates ;; 12) neofetch; pause ;; 13) DiskAnalyzer ;; 14) SnapshotManager ;; 15) NetworkSpeed ;;
        20) GuidedRescue ;; 21) RunBootRepairGUI ;; 22) GrubRescueCheatSheet ;; 23) $SAI testdisk ;; 24) Graphics_Menu ;; 
        30) Ollama_Setup ;; 31) Podman_Menu ;; 32) InstallDocker ;; 34) InstallGo ;;
        35) Ollama_Serve_Window ;; 36) MonitorGPU_Window ;;
        40) read -r -p "User: " u; sudo adduser "$u"; sudo usermod -aG sudo "$u" ;;
