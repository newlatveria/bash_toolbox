...
LXDsetup(){
    ...
    # Added qemu-utils for VM support
...
MakeVM(){
    ...
    wait4ip
    lastmessage="VM $newcon is now running."
}

MakeWindowsVM(){
    echo "Note: Windows VMs require a custom ISO and specialized drivers."
    contname
    lxc init "$newcon" --vm --empty
    lxc config set "$newcon" limits.cpu 4
    lxc config set "$newcon" limits.memory 4GB
    lxc config device add "$newcon" install disk source=/path/to/windows.iso boot.priority=10
    lastmessage="Windows VM shell $newcon initialized."
}
...
LXDmenu(){
	MenuTitle=" LXD - Options (Pop!_OS + VM Support)"

LXD_menu(){
	echo -e "\n $MenuTitle \n-----------------------------------"
	echo "10. Install LXD (Snap)        20. List All (Detailed)"
	echo -e "${ORANGE}30. Create Ubuntu 22.04         31. Create Ubuntu 20.04 ${STD}"
	echo -e "${ORANGE}32. Create App Server           35. CREATE VIRTUAL MACHINE (VM)${STD}"
    echo "40. Nginx      41. Gogs       42. ROR        45. GitLab"
    echo "44. GoLang     46. Ruby       47. WIN VM SHELL"
	...
	echo "60. List Containers             61. Map host folder"
	echo "62. Show devices                63. Remove device"
	echo "70. Create Ephemeral            80. Detailed Info"
	echo -e "${MAGENTA}90. Delete Container/VM         91. Start Container/VM${STD}"
	echo -e "${MAGENTA}95. Stop Container/VM           96. Stop All / Shutdown${STD}"
	echo -e "${MAGENTA}97. Restart Container/VM        99. Exit${STD}"
	echo "-------------------------------" 
    echo -e " Status: $lastmessage " 
    echo "-------------------------------"
	
	RunningList=$(lxc list status=running --format csv -c n)
	if [ -z "$RunningList" ]; then
		echo -e "${RED}No running instances.${STD}"
	else
		echo "Running Instances (Containers & VMs):"
		echo "$RunningList" | sed 's/^/- /'
	fi
    jobs -l
}
